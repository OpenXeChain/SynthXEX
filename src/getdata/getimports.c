	  ...
	  if(((instructionBuffer[2] & 0xFC1FFFFF) == expectedInstructions[2])
	      ...
	      if(instructionBuffer[3] == expectedInstructions[3])
		{
		  ...
		  for(uint32_t i = 0; i < peData->peImportInfo.tableCount; i++)
		    {
		      if(peData->peImportInfo.tables[i].importCount == peData->peImportInfo.tables[i].branchStubCount)
			{
...
int getImports(FILE *pe, struct peData *peData)
{
  ...
  for(uint16_t i = 0; i < peData->sections.count; i++)
	...
	{
	  break; // We have all the branch stubs there could possibly be
	}

      if(!(peData->sections.section[i].permFlag & XEX_SECTION_CODE))
      	{
      	  continue; // Skip the current section if it's not executable
      	}

      // Seek to the start of the current section to begin reading
      if(fseek(pe, peData->sections.section[i].offset, SEEK_SET) != 0)
	{
	  return ERR_FILE_READ;
	}

      // While we're within the bounds of the current section, and there's at least 4 instructions space left
